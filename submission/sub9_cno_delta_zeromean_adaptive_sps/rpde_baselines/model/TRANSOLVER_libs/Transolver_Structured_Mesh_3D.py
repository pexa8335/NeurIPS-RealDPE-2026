...
class Model(nn.Module):
    ...
    def forward(self, x,fx=None, T=None):
        ...
        x = x.reshape(x.shape[0], -1, input_shape[-1])
        if self.unified_pos:
        ...
        if T is not None:
            Time_emb = timestep_embedding(T, self.n_hidden)  # [batch_size, n_hidden]
            Time_emb = self.time_fc(Time_emb)  # [batch_size, n_hidden]
            Time_emb = Time_emb.unsqueeze(1).expand(-1, x.shape[1], -1)  # [batch_size, H*W*D, n_hidden]
            fx = fx + Time_emb

        for block in self.blocks:
            
            fx = block(fx)
        input_shape[-1] = self.out_dim

        return fx.reshape(tuple(input_shape))

    def train_loss(self, input, target):
        pred = self.forward(input,fx=None)
