"""RealPDE Track 1 Sim2Real submission: CNO baseline + calibrated SPS intervals.

Wraps the organizers' sim_real_fine-tuned CNO checkpoint behind the Track 1
predict() API, and returns per-element lower/upper bounds for sps_score.

Bounds: prediction +/- Q * RMS_RESIDUAL, a constant half-width calibrated on
train_real windows with the official scorer (see runs/sps/cno_const.npz).
The constant band beats the scorer's default +/-5%*|pred| band, which collapses
to zero width where the prediction is near zero. Q is set slightly above the
in-sample optimum because the hidden evaluation windows are out-of-sample for
this checkpoint (larger residuals).

Archive layout (all at the zip ROOT):
    submission.py       <- this file
    sim_real_cno.pth    <- checkpoint
    load_baseline.py
    rpde_baselines/
    _vendor/
"""
...
_CKPT_PATH = os.path.join(_HERE, "sim_real_cno.pth")
