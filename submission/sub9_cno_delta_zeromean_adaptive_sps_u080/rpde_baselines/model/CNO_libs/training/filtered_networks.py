...
import numpy as np
# scipy is imported lazily inside design_lowpass_filter(): it is only needed to
# design the anti-aliasing filters of the filtered-LReLU path (activation
# 'lrelu'). The released CNO baselines use activation 'LeakyReLU', so importing,
# building and running them requires nothing beyond torch + numpy.
import torch
...
class SynthesisLayer(torch.nn.Module):
    ...
    def design_lowpass_filter(numtaps, cutoff, width, fs, radial=False):
        ...
        import scipy.optimize  # noqa: F401  (kept for parity with upstream)
        import scipy.signal
