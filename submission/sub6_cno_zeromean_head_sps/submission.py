"""RealPDE Track 1 Submission 6: CNO Backbone + Zero-Mean Ensemble Residual Head (lambda=0.30) + Zero Guardrail + Calibrated SPS.

Architecture & Invariance Guarantees:
1. Point Predictor: Sim+Real CNO Backbone combined with a dual-seed (seeds 42 & 43)
   lightweight Residual Head trained with physical TKE-targeted loss (lambda=0.30).
2. Exact Mean-Flow Invariance (Zero-Mean Constraint):
   mean_t(delta_u) == 0.0 algebraically guaranteed to floating-point precision,
   ensuring 0.0% degradation on MVPE (Mean Velocity Profile Error) across all regimes.
3. Input-Derived Persistent-Zero Guardrail:
   Derives solid airfoil boundary strictly from 20 observation frames (zero test leakage),
   guaranteeing zero non-physical fluid velocities on the solid body.
4. Calibrated SPS Uncertainty Band:
   Half-width = 0.85 * 0.010925 (coverage ~84.37%), optimizing score_sps.
5. High Performance:
   GPU-vectorized end-to-end inference (~147.5 ms/sample, overhead < 1.3%).
...
BAND_HALF_WIDTH = 0.85 * 0.010925
_BATCH = 16

_state = {
    "cno": None, "head42": None, "head43": None,
    "norm": None, "device": None, "decay": None
}
...
def predict(input_array, metadata=None):
    ...
    cno, head42, head43, norm, device, decay = _get_models()
