...
SUB_DIR = r"D:\Project\NeurIPS\CCCCCC\submissions\sub3_cno_head_sps"
SUB_ZIP = r"D:\Project\NeurIPS\CCCCCC\submissions\sub3_cno_head_sps.zip"
SRC_SUB1 = r"D:\Project\NeurIPS\CCCCCC\submissions\sub1_cno_sps"
KAGGLE_CKPT_DIR = r"D:\Project\NeurIPS\kaggle_output\tke_test\loss_screen"

os.makedirs(SUB_DIR, exist_ok=True)

print("Step 1: Copying base CNO dependencies...")
for item in ["_vendor", "rpde_baselines", "load_baseline.py", "sim_real_cno.pth"]:
    src = os.path.join(SRC_SUB1, item)
...
print("\nStep 2: Preparing compact head ensemble checkpoint...")
ckpt42 = torch.load(os.path.join(KAGGLE_CKPT_DIR, "primary_lambda_0.1_seed42.pth"), map_location="cpu", weights_only=False)
ckpt43 = torch.load(os.path.join(KAGGLE_CKPT_DIR, "primary_lambda_0.1_seed43.pth"), map_location="cpu", weights_only=False)

head_bundle = {
    "state_dict_42": ckpt42["state_dict"],
    "state_dict_43": ckpt43["state_dict"],
    "norm_mean": ckpt42["norm_mean"].astype(np.float32) if isinstance(ckpt42["norm_mean"], np.ndarray) else ckpt42["norm_mean"],
    "norm_std": ckpt42["norm_std"].astype(np.float32) if isinstance(ckpt42["norm_std"], np.ndarray) else ckpt42["norm_std"],
    "residual_std": ckpt42["residual_std"].astype(np.float32) if isinstance(ckpt42["residual_std"], np.ndarray) else ckpt42["residual_std"],
}

head_out_path = os.path.join(SUB_DIR, "cno_head_weights.pth")
torch.save(head_bundle, head_out_path)
print(f"  Saved {head_out_path} ({os.path.getsize(head_out_path) / 1024:.1f} KB)")

print("\nStep 3: Creating submission.py...")
submission_py_content = '''"""RealPDE Track 1 Submission 3: CNO Backbone + Ensemble Residual Head + Calibrated SPS.
...
sub_py_path = os.path.join(SUB_DIR, "submission.py")
with open(sub_py_path, "w", encoding="utf-8") as f:
    f.write(submission_py_content)
print(f"  Wrote {sub_py_path}")

print("\nStep 4: Packaging sub3_cno_head_sps.zip...")
...
with zipfile.ZipFile(SUB_ZIP, "w", zipfile.ZIP_DEFLATED) as z:
    for root, dirs, files in os.walk(SUB_DIR):
        for f in files:
            ...
            rel_path = os.path.relpath(full_path, SUB_DIR)
            z.write(full_path, rel_path)
            print(f"  + {rel_path}")

print(f"\nCreated: {SUB_ZIP} ({os.path.getsize(SUB_ZIP) / (1024**2):.2f} MB)")
